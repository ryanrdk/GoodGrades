...
const filterTasks = (items, tutorId) => items.filter(task => {
...
const getShortTutorById = tutorId => tutors
...
const Appointment = withStyles(styles, { name: 'Appointment' })(
  ({ classes, data, ...restProps }) => {
    const tutorClasses = createClassesByTutorId(
      data.tutorId, classes,
...
export default class Demo extends React.PureComponent {
  ...
  getEvents(){
    fetch(
      `https://good-grades-server.herokuapp.com/api/events`,
      {
        method: 'GET',
        headers: {
  ...
  render() {
    const {
      data, currentDate, currentViewName, currentTutor,
    } = this.state;
    console.log({data, currentTutor})
    return (
      <Paper>
        <Scheduler
          data={filterTasks(data, currentTutor)}
          height={660}
        >
          <ViewState
